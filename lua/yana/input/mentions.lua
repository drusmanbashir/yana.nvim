...
-- Mirrors commands.lua's shape.
--
-- M.get_mentions(panel) -> { { description, command, details, callback? }, ... }
-- M.extract_mentions(text) -> { new_content, enable_diagnostics }
--
-- Deliberately NO @codebase: yana has no repo map, so shipping it would
-- be a mention that lies.
--
...
local function insert_paths(panel, paths)
	...
	require("yana.panel.ui").insert_at_cursor(panel.prompt_buf, views.prompt(panel), table.concat(paths, " "))
end
...
-- @diagnostics is strip-and-flag: this feature's contract removes it
