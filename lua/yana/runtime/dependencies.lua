...
local dependencies_probe = require("yana.runtime.dependencies_probe")
...
local open_capture_rows = dependencies_probe.open_capture_rows
...
-- --model, --resume (lua/yana/agent/agent.lua's build_cmd()), or the --list-models format
...
--- (a wrapper script around the real install). Never "error": the raw
...
local function wrapper_warning_row(resolved, result)
  ...
  return row(
    ...
    "point cmd (or $YANA_AGENT_BIN / cmd_env) at a wrapper script rather than the bare resolved path "
      .. "-- one that exec's `node --use-system-ca index.js <real-install>` instead of the packaged "
      .. "binary directly"
  )
end
...
function M.check(mode, opts)
  ...
  if mode ~= "agentic" then
    local capture_ok, capture_mode = pcall(config.resolve_open_capture_mode)
    if capture_ok then
      for _, capture_row in ipairs(open_capture_rows(capture_mode, opts.include_optional_capture)) do
        rows[#rows + 1] = capture_row
      end
    else
      rows[#rows + 1] = row("open_capture_mode_invalid", "error", tostring(capture_mode), "set open_capture.mode to auto, overlayfs-fast, fuse-compat, or off")
    end; kernel_rows(rows)
...
function M.preflight(mode)
  ...
  local errors = {}
  for _, item in ipairs(M.check(mode, { probe_agent_version = false, include_optional_capture = false })) do
