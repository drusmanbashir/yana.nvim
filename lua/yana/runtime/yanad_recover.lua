...
local function build_changes(recovery)
	...
	for _, row in ipairs(recovery.bundle) do
		...
		if row.home_buffer_only then
			if type(row.review_before) ~= "string" then
				return nil, "buffer-only HOME recovery evidence is incomplete; the retained proposal was not reconstructed"
		end
			return nil,
				"buffer-only HOME review cannot be reconstructed after restart; the unsaved buffer baseline and private proposal remain retained in the turn evidence"
		end
...
function M.recover(session_id, deps, done)
	...
	require("yana.runtime.yanad").session_attach({ session_id = session_id }, request_id, function(ok, result)
