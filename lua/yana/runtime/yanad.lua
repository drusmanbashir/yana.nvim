...
function Client:_abort_version_epoch(next_version)
	-- A changed release is a hard boundary. Do not replay requests, queued
	-- prompts, callbacks or a panel's old session into the new runtime epoch.
	self._done = true
	self:_close()
	...
	for _, entry in pairs(pending) do
		...
		vim.schedule(function()
			entry.cb(false, "version_epoch_changed", {
				reason_code = "version_epoch_changed",
...
function Client:_reconnect()
	...
	local next_version = read_version()
	if next_version ~= self.version then
		return self:_abort_version_epoch(next_version)
	end
