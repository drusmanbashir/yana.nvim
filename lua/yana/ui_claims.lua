...
local function release_shadow_turn(p, reason, on_released)
  ...
  if not turn then
    if pass then
      require("yana.turn_lifecycle").finish_turn(pass)
  ...
  return preview.release(turn, function(ok, err)
    if not ok then
      ...
      if on_released then on_released(false, err) end
      return
    end

    -- The ACK is the ownership boundary. Only now may the panel forget the
    -- exact review session or expose the queue to a following turn.
    if p.shadow_turn == turn then
      p.shadow_turn = nil
      p.shadow_pass = nil
      if pass and p.turn_pass == pass then
        emit_turn_end(p, pass, reason, p.turn_end_outcome and p.turn_end_outcome[pass.generation])
        p.turn_pass = nil
      end
