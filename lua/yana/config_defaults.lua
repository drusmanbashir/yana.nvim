...
local defaults = {
  ...
  backends = require("yana.config_defaults_backends"),
  ...
  -- (external-roots.md, capture-set ruling). Missing, non-ancestor, and
  -- state-root-containing entries are normal misses; the workspace itself is
  -- the final outcome when none qualify. Operators with another directory
  -- layout can replace this list without changing write_roots.
  ...
  --
  -- This is compliance-by-request, not enforcement: a vendor model can honor it or
  -- ignore it, and a hostile or careless agent binary is not bound by it at all. The
  -- jail layer (design step 2 -- the overlay's shell surface refusing execution-class
  -- argv in inline mode) is the enforcement that holds regardless of what the model
...
Agent mode: when the user asks to populate, add, change, or give an example in a file, EDIT that file with the Edit File tool immediately — do not only reply in chat or ask whether to paste. Use minimal diffs at the referenced line numbers. When a visual selection is attached, prefer edits inside the stated edit zone; out-of-zone edits may be rejected or flagged before review. The user reviews each edit as inline hunks in the open file (cr/ca/cf) before it is final. Propose edits only — never run compilers, test suites, or import/smoke checks; the user's hunk-by-hunk review is the validation step here, not a shell command. If validation like that is actually needed, say so and let the user switch to agentic mode, where it belongs.
  ...
  mappings = {
    ...
    -- Global, set from any buffer when setup() runs; off by default.
    toggle = false,
    ask = false,
  ...
  inline_edit = {
	...
	-- Session registry + transcripts (lua/yana/sessions.lua). An empty table: `dir` unset
	-- means stdpath("data")/yana.
	-- Session registry + transcripts (lua/yana/sessions.lua). `dir` unset means
