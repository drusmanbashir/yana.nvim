...
-- Park a submit made before the panel has a daemon session. Each submit keeps
-- its own text, captured now (the prompt is cleared, as a busy-panel submit
-- is), in p.yanad_submit_queue in submit order, so a second submit during a
-- cold start queues behind the first instead of replacing it. Returns false
...
-- buffer.
  ...
  if config.overlay_mode() and not p.yanad_session_id then
    if p.yanad_session_pending then
      ...
      --
      -- Every submit is kept, in order (park_submit): two submits before an
      -- answer are two turns, exactly as the `pending` branch above behaves.
      -- The lifecycle callback takes the whole queue BEFORE it schedules the
      -- first re-submit (ui_panel_lifecycle.lua fire_parked_turn), so no
      -- parked submit can fire twice.
      park_submit(p, opts)
