...
function M.new(deps)
  ...
  local function check_recovery(opts)
    ...
    require("yana.yanad").status({}, request_id, function(ok, status)
      ...
      require("yana.yanad_recover").note_discovery_prunes(status.pruned)
      ...
      local function delete_session(choice, after)
        ...
        require("yana.yanad").session_delete({ session_id = choice.session_id }, req, function(ok, res)
          if not panel_is_current(target, target_id, workspace) then
            recovery_picker_open = false
            if after then
              after(false)
