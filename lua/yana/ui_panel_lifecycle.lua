...
function M.new(deps)
  ...
  -- Spend the submits parked in `p.yanad_submit_queue`, if any. THE ONE
  ...
  -- that stops a parked submit being sent twice exists in exactly one place.
  ...
  local function fire_parked_turn(p)
    ...
    vim.schedule(function()
      -- Submit order is kept: the rest join the panel queue BEFORE the first
      -- is launched, so a first that finds the panel busy is inserted at the
      -- queue head ahead of them (ui_submit.lua) and each drains as the turn
      -- before it ends. from_parked marks the ONE launch this queue produces
      -- directly; submit_panel reads only `text` off opts, so the flag changes
      -- nothing in the product and lets a row count parked fires apart from
  ...
  local function start_session_create(p)
    ...
    yanad.session_create({
      ...
      if ok and type(res) == "table" and res.session_id then
        ...
        -- p.yanad_submit_queue is deliberately LEFT SET. Those are the turns the
        ...
        -- the loss. Nothing fires twice: fire_parked_turn above takes the whole
        -- queue out BEFORE scheduling the re-submit, so the only path that
        -- spends it also empties it.
      end
    end)
  end
