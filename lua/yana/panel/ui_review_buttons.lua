...
local function live_pool_for_panel(panel)
  ...
  return require("yana.turn.turn_bind").live_pool()
end
...
function M.attach_tab(tab)
  ...
  return attach_pool(require("yana.turn.turn_bind").live_pool(), tab, panels[1])
end
...
function M.open(pool)
  ...
  for _, tab in ipairs(vim.api.nvim_list_tabpages()) do
    ...
    if #panels > 0 then
      ...
      local live = require("yana.turn.turn_bind").live_pool()
...
vim.api.nvim_create_autocmd({ "WinResized", "VimResized" }, {
  ...
  callback = function()
    ...
    if not require("yana.turn.turn_bind").live_pool() then return end
