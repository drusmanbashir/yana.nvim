...
-- `reconcile_pending` is handed to yana.ui_winbar as a plain function dep captured at construction, so ui.lua
-- MUST instantiate this module before that call.
...
-- deps.current_panel: cursor/MRU panel; deps.update_winbar: repaint one panel's chip; deps.turn_ledger: per-(panel,gen) ledger lookup.
function M.new(deps)
  ...
  --- The `ask` answer's artifact as an instruction; extraction is the renewal module's so resend and brief agree.
  ...
  --- The brief AS THE OPERATOR SEES IT at switch time, not the raw transcript.
  ...
  -- Invariant capture: the pending number shown to the user vs the number the review engine holds.
  ...
  -- Set a panel's mode programmatically. Subject to the same lock as the <M-t> toggle: callers go through here,
  -- never assign p.mode. Returns true when the panel is in `mode` on return, false when refused.
  ...
  -- Whether `p` can produce an EDIT at all (true for inline/agentic, false for ask). Answering must never pick a
  -- confinement level: only set_mode may assign config.options.mode ("agent" was once an alias that silently
  -- promoted a configured `inline` panel to unconfined `agentic`).
