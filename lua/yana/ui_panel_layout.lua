...
function M.new(deps)
  ...
  local prompt_api = require("yana.ui_panel_layout_prompt").new({
    state = S,
    win_valid = win_valid,
    cur_tab = cur_tab,
  ...
  local function hide_panel_windows(p, tab)
    tab = tab or cur_tab()
    pcall(require("yana.ui_review_buttons").detach_windows, tab, p)
  ...
  local function attach_review_strip(p, tab)
    pcall(require("yana.ui_review_buttons").attach, p, tab)
  end
  ...
  local function open_windows(p, tab)
    ...
    S._prompt_layout_building = (S._prompt_layout_building or 0) + 1
    local ok, err = xpcall(function()
      ...
      if anchor and policy.is_split() then
        ...
        pcall(require("yana.ui_review_buttons").detach_windows, tab, host_panel)
      ...
      if not stacked then
        local want = require("yana.ui_geometry").desired(tab) or compute_width()
      ...
      local geometry = require("yana.ui_geometry")
      geometry.setup()
    ...
    S._prompt_layout_building = (S._prompt_layout_building or 1) - 1
    if not ok then
      error(err)
    end
  end
  ...
  local nav
  nav = require("yana.ui_panel_layout_nav").new({
    state = S,
    panels = panels,
    win_valid = win_valid,
