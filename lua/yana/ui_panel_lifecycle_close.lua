...
function M.new(deps)
  ...
  local function quit_panel(p)
    ...
    local strip_tabs = {}
    pcall(function()
      strip_tabs = require("yana.ui_review_buttons").detach_panel(p) or {}
    end)
    ...
    if S.last_panel == p then
      S.last_panel = nil
    end
    if S.last_focused_panel == p then
      S.last_focused_panel = nil
    end
    if S.primary_panel == p then
      S.primary_panel = nil
    end
    ...
    for _, tab in ipairs(tabs) do
      ...
      preserve_survivor_prompt(tab, S.last_panel)
    end
    p.closing = false
    ...
    for tab in pairs(reattach) do
      vim.schedule(function()
        if vim.api.nvim_tabpage_is_valid(tab) then
          pcall(require("yana.ui_review_buttons").attach_tab, tab)
        end
      end)
    end
    return true
